...
standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels)
{
	...
	foreach(lc, root->join_rel_level[1])
		{
...
join_search_one_level(PlannerInfo *root, int level)
{
	...
	// printf("level %d\n",level);
	int				save_nestlevel;
	bool enable_all_join_for_zero_hint = false;
	if (current_hint_state->join_hint_level[level]==NULL){
		// printf("core.c:345 enable_all !\n");
		enable_all_join_for_zero_hint = true;
	}
	if (enable_all_join_for_zero_hint){
		save_nestlevel = NewGUCNestLevel();
		set_join_config_options(ENABLE_ALL_JOIN,
								current_hint_state->context);
	}
	foreach(r, joinrels[level - 1])
	{
		RelOptInfo *old_rel = (RelOptInfo *) lfirst(r);
		if (old_rel->cheapest_total_path->total_cost >disable_cost) 
			continue;
 ...
 */
void
print_relids_yx(PlannerInfo *root, Relids relids)
{
	int			x;
	bool		first = true;
						  ...
						  ListCell *other_rels)
{
	...
	for_each_cell(l, other_rels)
	{
		...
		{
			// printf("make_rels_by_clause_joins ");
			// print_relids_yx(root, old_rel->relids);
			// print_relids_yx(root, other_rel->relids);
			// printf("\n");
