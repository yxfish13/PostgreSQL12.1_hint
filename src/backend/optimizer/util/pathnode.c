...
#define Total_Disable_Cost(x) (disable_cost*(int)(x/disable_cost))
#define Remove_Disable_Cost(x) (x-disable_cost*(int)(x/disable_cost))
...
#define CONSIDER_PATH_STARTUP_COST(p)  \
	 ...
	 */
	if ((path1->total_cost- Total_Disable_Cost(path2->total_cost)) > (path2->total_cost - Total_Disable_Cost(path2->total_cost)) * fuzz_factor)
	{
		/* path1 fuzzily worse on total cost */
		if (CONSIDER_PATH_STARTUP_COST(path1) &&
			(path2->startup_cost- Total_Disable_Cost(path2->startup_cost)) > (path1->startup_cost- Total_Disable_Cost(path2->startup_cost)) * fuzz_factor)
		{
	...
	if ((path2->total_cost- Total_Disable_Cost(path1->total_cost)) > (path1->total_cost- Total_Disable_Cost(path1->total_cost)) * fuzz_factor)
	{
		/* path2 fuzzily worse on total cost */
		if (CONSIDER_PATH_STARTUP_COST(path2) &&
			(path1->startup_cost- Total_Disable_Cost(path1->startup_cost)) > (path2->startup_cost- Total_Disable_Cost(path1->startup_cost)) * fuzz_factor)
		{
	...
	if ((path1->startup_cost- Total_Disable_Cost(path1->startup_cost)) > (path2->startup_cost- Total_Disable_Cost(path1->startup_cost)) * fuzz_factor)
	{
		/* ... but path1 fuzzily worse on startup, so path2 wins */
		return COSTS_BETTER2;
	}
	if ((path2->startup_cost- Total_Disable_Cost(path1->startup_cost)) > (path1->startup_cost- Total_Disable_Cost(path1->startup_cost)) * fuzz_factor)
	{
				  ...
				  List *pathkeys, Relids required_outer)
{
	...
	foreach(p1, parent_rel->pathlist)
	{
		 ...
		 */
		if ((total_cost - Total_Disable_Cost(total_cost)) > (old_path->total_cost - Total_Disable_Cost(total_cost)) * STD_FUZZ_FACTOR)
		{
			/* new path can win on startup cost only if consider_startup */
			if ((startup_cost- Total_Disable_Cost(startup_cost)) > (old_path->startup_cost- Total_Disable_Cost(startup_cost)) * STD_FUZZ_FACTOR ||
				!consider_startup)
			{
...
add_partial_path(RelOptInfo *parent_rel, Path *new_path)
{
		 ...
		 p1 = p1_next)
	{
		...
		if (keyscmp != PATHKEYS_DIFFERENT)
		{
			if ((new_path->total_cost - Total_Disable_Cost(new_path->total_cost)) > (old_path->total_cost - Total_Disable_Cost(new_path->total_cost)) * STD_FUZZ_FACTOR)
			{
				/* New path costs more; keep it only if pathkeys are better. */
				if (keyscmp != PATHKEYS_BETTER1)
					accept_new = false;
			}
			else if ((old_path->total_cost- Total_Disable_Cost(new_path->total_cost)) > (new_path->total_cost- Total_Disable_Cost(new_path->total_cost))
					 * STD_FUZZ_FACTOR)
			{
	...
	foreach(p1, parent_rel->partial_pathlist)
	{
		...
		if (keyscmp != PATHKEYS_DIFFERENT)
		{
			if ((total_cost- Total_Disable_Cost(total_cost)) > (old_path->total_cost- Total_Disable_Cost(total_cost)) * STD_FUZZ_FACTOR &&
				...
				return false;
			if ((old_path->total_cost- Total_Disable_Cost(total_cost)) > (total_cost- Total_Disable_Cost(total_cost)) * STD_FUZZ_FACTOR &&
				keyscmp != PATHKEYS_BETTER2)
				return true;
		}
	}
